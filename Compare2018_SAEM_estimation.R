...
{ 
  ...
  # nu.fixed: if FALSE, the parameter nu is estimated; TRUE, the parameter nu is fixed.
  # nu : if ST and SSL : numeric ; if SCN: a vector c(x,y). 
  ...
  while (critval < 3 && critval2 < 3)
  {
    ...
    if(nu.fixed==FALSE)
    {
      if(dist=="ST")
      {
        nu <- optimize(f=logVerosCens.nu, interval=c(2.1,150),tol=0.00001,theta=c(betas,sigma2,lambda),y=y,X=X,cc=cc,cens=cens,dist=dist)$minimum
      }
      if(dist=="SSL")
      {
...
#  print(round(se,4))
  ...
  likelihood <- logVerosCens(teta.out, y, X, cc, cens, dist)   
  
  end.time <- Sys.time()
  time.taken <- end.time - start.time
  
  out <- list(teta=teta.out, likelihood= likelihood, time=time.taken, iter=count, EP=se, criterio=max(criterio,criterio2),EU=EU,EUY=EUY,EUY2=EUY2,EUT=EUT,EUT2=EUT2,EUYT=EUYT)
  return(out)   
}
